...
$heartbeatCurrent = $heartbeatVersion !== '' && version_compare($heartbeatVersion, $currentVersion, '==');
$updatedDeferred = $heartbeatFresh && $heartbeatCurrent && $heartbeatStatus === 'updated_deferred';
?><!doctype html>
...
<style>*{box-sizing:border-box;min-width:0}html,body{width:100%;max-width:100%;overflow-x:hidden}body{margin:0;background:#f4f7fb;color:#172033;font-family:Tahoma,Arial,sans-serif}.wrap{width:100%;max-width:920px;margin:auto;padding:16px;overflow-x:hidden}.top,.card,.row,form{max-width:100%;min-width:0}.top{display:flex;justify-content:space-between;gap:10px;align-items:center}.card{background:#fff;border:1px solid #e2e8f0;border-radius:18px;padding:17px;margin-top:14px;overflow:hidden}.btn{max-width:100%;border:0;border-radius:11px;padding:11px 14px;background:#1769ff;color:#fff;font-weight:700;text-decoration:none;display:inline-block;cursor:pointer;white-space:normal;overflow-wrap:anywhere}.btn.alt{background:#eef4ff;color:#174f9f}.msg{padding:12px;border-radius:12px;background:#ecfff5;color:#12633f;margin-top:12px;overflow-wrap:anywhere}.err{padding:12px;border-radius:12px;background:#fff0f0;color:#a11;margin-top:12px;overflow-wrap:anywhere}.info{padding:12px;border-radius:12px;background:#edf6ff;color:#174f9f;margin-top:12px;overflow-wrap:anywhere}.code{max-width:100%;direction:ltr;text-align:left;white-space:pre-wrap;overflow-wrap:anywhere;word-break:break-all;background:#101827;color:#eef5ff;padding:13px;border-radius:12px;font:12px monospace;max-height:420px;overflow-x:hidden;overflow-y:auto}.good{color:#087857}.bad{color:#b42318}.muted{color:#68748a;font-size:13px;overflow-wrap:anywhere}.row{display:flex;gap:8px;flex-wrap:wrap}.row>*{max-width:100%;min-width:0}@media(max-width:700px){.wrap{padding:11px}.top{align-items:flex-start;flex-direction:column}.row{width:100%}.btn,.row>a,.row>form,.row>form .btn{width:100%;text-align:center}}</style></head><body><div class="wrap">
...
<?php if($updatedDeferred):?><div class="info"><b>آپدیت Backend داخل Cron با موفقیت انجام شده است.</b><br>وضعیت <span dir="ltr">updated_deferred</span> خطا نیست. Trade عمداً پس از جایگزینی فایل‌ها همان Process را متوقف می‌کند؛ حالا یک Tick دیگر اجرا کن تا نسخه <?=h($currentVersion)?> اجرای کامل معاملات را ثبت کند.</div><?php endif;?>
<div class="card"><h2><?=$updatedDeferred?'اجرای Tick بعدی':'اجرای دستی امن'?></h2><p><?=$updatedDeferred?'این اجرا باید اولین bot_run کامل نسخه جدید را بسازد.':'این دکمه همان <code>cron/tick.php</code> را با PHP CLI هاست در پس‌زمینه اجرا می‌کند؛ بنابراین نتیجه دقیقاً همان چیزی است که cPanel Cron باید اجرا کند.'?></p><form method="post"><input type="hidden" name="csrf" value="<?=h((string)$_SESSION['csrf'])?>"><button class="btn" type="submit" <?=$canExec?'':'disabled'?>><?=$updatedDeferred?'اجرای Tick بعدی همین حالا':'اجرای Cron همین حالا'?></button></form><?php if(!$canExec):?><p class="bad">exec در این هاست غیرفعال است؛ از Command زیر در cPanel استفاده کن.</p><?php endif;?></div>
<div class="card"><h2>Heartbeat</h2><?php if($heartbeat):?><p class="<?=$heartbeatFresh?'good':'bad'?>">آخرین شروع واقعی Cron: <?=h($heartbeatAge===null?'نامشخص':$heartbeatAge.' ثانیه قبل')?> — وضعیت: <b><?=h($heartbeat['status']??'unknown')?></b> — نسخه: <b><?=h($heartbeatVersion!==''?$heartbeatVersion:'?')?></b></p><div class="code"><?=h(json_encode($heartbeat,JSON_PRETTY_PRINT|JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES))?></div><?php else:?><p class="bad">هنوز heartbeat ثبت نشده؛ یعنی نسخه دارای تشخیص جدید Cron هنوز اجرا نشده است.</p><?php endif;?></div>
