...
?>
<div class="card exchange"><div class="sectionTitle"><div><h2><?=$name?></h2><div class="muted"><?=$key==='nobitex'?'Profit-First • Full Universe • Adaptive Position Sizing • Pending Watchdog':'کنترل مستقل صرافی'?></div></div><div class="badges"><span class="badge <?=$e['credentials_configured']?'on':'off'?>">API <?=$e['credentials_configured']?'READY':'OFF'?></span><span class="badge <?=$e['bot_enabled']?'on':'off'?>">Bot <?=$e['bot_enabled']?'ON':'OFF'?></span><span class="badge <?=$e['live_execution_enabled']?'on':'off'?>">Live <?=$e['live_execution_enabled']?'ON':'OFF'?></span></div></div>
