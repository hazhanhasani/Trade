...
<section class="panel"><div class="panel-head"><div><h2><?=$name?></h2><p><?=$key==='nobitex'?'صرافی اصلی • تحلیل همه بازارها • اندازه خرید تطبیقی':'صرافی ثانویه'?></p></div><div class="status-line"><span class="badge <?=$e['credentials_configured']?'good':'bad'?>">API <?=$e['credentials_configured']?'متصل':'قطع'?></span><span class="badge <?=$e['bot_enabled']?'good':'bad'?>">ربات <?=$e['bot_enabled']?'روشن':'خاموش'?></span><span class="badge <?=$e['live_execution_enabled']?'good':'bad'?>">ارسال واقعی <?=$e['live_execution_enabled']?'روشن':'خاموش'?></span></div></div>
...
<div class="decision"><b>آخرین تصمیم ربات</b><br><?=h(decisionLine($e['last_decision']??null))?></div>
<?php if($positions):?><div class="positions" style="margin-top:10px"><?php foreach($positions as $pos):$sym=strtoupper((string)$pos['symbol']);$an=positionAnalytics($pos,$latestMap[$sym]??null);?><div class="position"><div class="position-head"><b><?=h($pos['symbol'])?></b><span class="badge good"><?=h($pos['status'])?></span></div><div class="position-grid"><div class="mini"><span>قیمت ورود</span><b><?=fmt($pos['entry_price'],8)?></b></div><div class="mini"><span>قیمت فعلی پایش</span><b><?=fmt($an['current'],8)?></b></div><div class="mini"><span>سود/زیان خالص</span><b class="<?=$an['net_pct']>=0?'ok':'bad-text'?>"><?=fmt($an['net_pnl'],2)?> <?=h($pos['quote_asset'])?> • <?=fmt($an['net_pct'],2)?>%</b></div><div class="mini"><span>حد سود / حد ضرر</span><b><?=fmt($pos['take_profit'],6)?> / <?=fmt($pos['stop_loss'],6)?></b></div></div></div><?php endforeach?></div><?php else:?><div class="empty">معامله بازی وجود ندارد.</div><?php endif?>
...
<section class="panel soft"><div class="panel-head"><div><h2>تنظیمات ساده معاملات و ریسک</h2><p>زیر هر گزینه نوشته شده دقیقاً چه کاری انجام می‌دهد. تنظیمات API و روشن/خاموش کردن ربات در بخش «صرافی‌ها» است.</p></div><span class="badge info">قابل تنظیم</span></div>
<div class="notice info"><b>پیشنهاد برای استفاده معمول:</b> اگر مطمئن نیستی، سطح ریسک را روی «متعادل» نگه دار و فقط تعداد معاملات هم‌زمان و سقف کل سرمایه درگیر را با توجه به سرمایه‌ات تنظیم کن.</div>
<form method="post"><input type="hidden" name="csrf" value="<?=$csrf?>"><input type="hidden" name="action" value="save_settings"><div class="field-grid">
<div class="field"><label>بازار اصلی برای معامله</label><select name="quote_asset"><option value="IRT" <?=$settings['quote_asset']==='IRT'?'selected':''?>>تومان (IRT)</option><option value="USDT" <?=$settings['quote_asset']==='USDT'?'selected':''?>>تتر (USDT)</option></select><span class="field-help">ربات اول فرصت‌های این نوع بازار را در اولویت قرار می‌دهد.</span></div>
<div class="field"><label>سطح ریسک کلی</label><select name="risk_profile"><option value="safe" <?=$settings['risk_profile']==='safe'?'selected':''?>>کم‌ریسک</option><option value="balanced" <?=$settings['risk_profile']==='balanced'?'selected':''?>>متعادل</option><option value="aggressive" <?=$settings['risk_profile']==='aggressive'?'selected':''?>>پرریسک</option></select><span class="field-help">روی سخت‌گیری ورود و مدیریت سرمایه اثر می‌گذارد؛ «متعادل» انتخاب پیش‌فرض مناسب‌تری است.</span></div>
<div class="field"><label>حداقل امتیاز خرید در Bitpin</label><input type="number" min="35" max="90" name="min_signal_score" value="<?=h($settings['min_signal_score'])?>"><span class="field-help">فقط مربوط به Bitpin است؛ هرچه بیشتر باشد، ورود سخت‌گیرانه‌تر می‌شود. نوبیتکس از این امتیاز استفاده نمی‌کند.</span></div>
<div class="field"><label>درصد سرمایه پیشنهادی برای هر خرید</label><input type="number" step="0.1" min="0.5" max="20" name="position_percent" value="<?=h($settings['position_percent'])?>"><span class="field-help">مثلاً 3 یعنی ربات برای یک خرید جدید حداکثر حدود ۳٪ از ارزش پورتفو را هدف می‌گیرد، مگر کنترل‌های دیگر آن را کمتر کنند.</span></div>
<div class="field"><label>درصد واقعی خرید بعدی نوبیتکس</label><input type="text" value="<?=h(fmt($settings['nobitex_effective_position_percent']??0,2))?>% — خودکار" disabled><span class="field-help">این عدد خودکار از سقف کل سرمایه و تعداد معاملات مجاز محاسبه می‌شود و قابل ویرایش مستقیم نیست.</span></div>
<div class="field"><label>حداکثر سرمایه در یک معامله</label><input type="number" step="0.1" min="1" max="25" name="max_position_percent" value="<?=h($settings['max_position_percent'])?>"><span class="field-help">هیچ معامله جدیدی نباید بیشتر از این درصد از کل سرمایه را درگیر کند.</span></div>
<div class="field"><label>حداکثر کل سرمایه درگیر در نوبیتکس</label><input type="number" step="1" min="10" max="90" name="nobitex_portfolio_exposure_percent" value="<?=h($settings['nobitex_portfolio_exposure_percent'])?>"><span class="field-help">مثلاً 60 یعنی مجموع معاملات باز نوبیتکس نباید بیشتر از حدود ۶۰٪ کل پورتفو شود.</span></div>
<div class="field"><label>حداکثر تعداد معاملات باز هم‌زمان</label><input type="number" min="1" max="20" name="nobitex_max_positions" value="<?=h($settings['nobitex_max_positions'])?>"><span class="field-help">وقتی این تعداد پر شود، ربات خرید جدید عادی انجام نمی‌دهد مگر منطق جایگزینی فرصت فعال شود.</span></div>
<div class="field"><label>حداکثر سفارش‌های در انتظار</label><input type="number" min="1" max="5" name="nobitex_max_pending_orders" value="<?=h($settings['nobitex_max_pending_orders'])?>"><span class="field-help">تعداد سفارش‌هایی که ارسال شده‌اند اما هنوز کامل نشده‌اند. سقف کمتر، کنترل محافظه‌کارانه‌تری ایجاد می‌کند.</span></div>
<div class="field"><label>حداکثر زمان انتظار برای سفارش</label><input type="number" min="30" max="300" step="5" name="nobitex_pending_timeout_seconds" value="<?=h($settings['nobitex_pending_timeout_seconds'])?>"><span class="field-help">اگر سفارش تا این تعداد ثانیه تعیین تکلیف نشود، ربات آن را بررسی و در صورت نیاز لغو/همگام می‌کند.</span></div>
<div class="field"><label>فاصله اجباری بین معاملات یک ارز</label><input type="number" min="1" max="1440" name="cooldown_minutes" value="<?=h($settings['cooldown_minutes'])?>"><span class="field-help">بعد از معامله یک ارز، تا این مدت دوباره روی همان ارز وارد نمی‌شود تا از خرید و فروش پشت‌سرهم جلوگیری شود.</span></div>
<div class="field"><label>حد ضرر هر معامله</label><input type="number" step="0.1" min="0.5" max="15" name="stop_loss_percent" value="<?=h($settings['stop_loss_percent'])?>"><span class="field-help">اگر زیان معامله به این محدوده برسد، موتور مدیریت ریسک برای خروج اقدام می‌کند.</span></div>
<div class="field"><label>هدف سود پایه هر معامله</label><input type="number" step="0.1" min="0.5" max="50" name="take_profit_percent" value="<?=h($settings['take_profit_percent'])?>"><span class="field-help">هدف سود پایه است؛ منطق سودمحور و قفل سود ممکن است بر اساس شرایط بازار زودتر یا هوشمندانه‌تر خارج شود.</span></div>
<div class="field"><label>حداکثر زیان مجاز روزانه</label><input type="number" step="0.1" min="1" max="15" name="daily_loss_limit_percent" value="<?=h($settings['daily_loss_limit_percent'])?>"><span class="field-help">پس از رسیدن زیان روز به این حد، خریدهای جدید برای محافظت از سرمایه متوقف می‌شوند.</span></div>
</div><div class="actions" style="margin-top:12px"><button class="btn">ذخیره تنظیمات</button></div></form>
<details class="help-box"><summary>معنی اصطلاحات رایج این پنل</summary><p><span class="term">پوزیشن / معامله باز:</span> خریدی که هنوز فروخته نشده است. <span class="term">سفارش در انتظار:</span> سفارشی که صرافی دریافت کرده ولی هنوز کامل نشده است. <span class="term">سود قابل معامله:</span> سود پیش‌بینی‌شده بعد از کم کردن کارمزد، فاصله خرید و فروش، لغزش قیمت و حاشیه اطمینان. <span class="term">درصد سرمایه درگیر:</span> بخشی از کل دارایی که در معاملات باز استفاده شده است. <span class="term">فاصله اجباری:</span> زمان استراحت بین دو معامله روی یک ارز برای جلوگیری از معامله بیش‌ازحد.</p></details>
</section>

<div class="panel-grid">
<?php foreach(['nobitex'=>'Nobitex','bitpin'=>'Bitpin'] as $key=>$name):$positions=$ex[$key]['active_positions']??[];$activeSet=activeSymbolSet($positions);?><section class="panel"><div class="panel-head"><div><h2>تحلیل‌های اخیر <?=$name?></h2><p>۳۰ مورد اخیر؛ این جدول نشان می‌دهد ربات چه دیده و آیا سفارشی ساخته یا نه.</p></div></div><div class="table-wrap"><table><thead><tr><th>زمان UTC</th><th>بازار</th><th>تصمیم</th><th><?=$key==='nobitex'?'سود قابل معامله':'امتیاز'?></th><th>اطمینان تحلیل</th><th>نتیجه</th></tr></thead><tbody><?php if(($data[$key]['signals']??[])===[]):?><tr><td colspan="6" class="empty">تحلیلی ثبت نشده است.</td></tr><?php endif?><?php foreach($data[$key]['signals']??[] as $r):$details=detailsOf($r);$action=strtolower((string)$r['action']);[$stateLabel,$stateClass]=signalStateFa($r,$activeSet);?><tr><td><?=h($r['created_at'])?></td><td><b><?=h($r['symbol'])?></b></td><td><?=h(match($action){'buy'=>'خرید','sell'=>'فروش','hold'=>'صبر',default=>strtoupper($action)})?></td><td><?php if($key==='nobitex'):?><?=isset($details['tradable_net_edge_percent'])?fmt($details['tradable_net_edge_percent'],3).'%':(isset($details['expected_net_edge_percent'])?fmt($details['expected_net_edge_percent'],3).'%':'—')?><?php else:?><?=h($r['score'])?><?php endif?></td><td><?=isset($details['confidence'])?h($details['confidence']).'%':'—'?></td><td><span class="badge <?=$stateClass?>"><?=h($stateLabel)?></span></td></tr><?php endforeach?></tbody></table></div></section><?php endforeach?>
</div>
