...
<div class="notice">نوبیتکس Score ندارد. ابتدا Net Edge پس از کارمزد، Spread و Slippage محاسبه می‌شود؛ سپس حاشیه اطمینان پویا بر اساس هزینه اجرا و نوسان کم می‌شود. فقط وقتی «Edge قابل معامله» مثبت بماند خرید انجام می‌شود. این حاشیه در خود سیگنال نمایش داده می‌شود و شرط مخفی نیست.</div>
<div class="info"><b>تغییر وضعیت سیگنال‌ها:</b> «پایش پوزیشن باز» یعنی خرید قبلاً انجام شده و ربات در هر چرخه فقط همان پوزیشن را دوباره ارزیابی می‌کند. سقف همزمان نوبیتکس اکنون تا ۲۰ پوزیشن قابل تنظیم است، ولی سقف درصد سرمایه درگیر همچنان مستقل عمل می‌کند.</div>
...
?>
<div class="card exchange"><div class="sectionTitle"><div><h2><?=$name?></h2><div class="muted"><?=$key==='nobitex'?'Profit-First • Full Universe • Adaptive Edge • بدون Score':'کنترل مستقل صرافی'?></div></div><div class="badges"><span class="badge <?=$e['credentials_configured']?'on':'off'?>">API <?=$e['credentials_configured']?'READY':'OFF'?></span><span class="badge <?=$e['bot_enabled']?'on':'off'?>">Bot <?=$e['bot_enabled']?'ON':'OFF'?></span><span class="badge <?=$e['live_execution_enabled']?'on':'off'?>">Live <?=$e['live_execution_enabled']?'ON':'OFF'?></span></div></div>
...
<div class="field"><label>درصد سرمایه هر ورود</label><input type="number" step="0.1" min="0.5" max="20" name="position_percent" value="<?=h($settings['position_percent'])?>"></div>
