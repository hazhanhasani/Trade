(()=>{
  ...
  function toggleTheme(){
    ...
    localStorage.setItem('trade-theme',root.dataset.theme);
    updateThemeButton();
  }

  let iranClockFormatter=null;
  try{
    iranClockFormatter=new Intl.DateTimeFormat('fa-IR-u-ca-persian',{
      timeZone:'Asia/Tehran',weekday:'long',year:'numeric',month:'2-digit',day:'2-digit',
      hour:'2-digit',minute:'2-digit',second:'2-digit',hour12:false
    });
  }catch(_){ iranClockFormatter=null; }
  function updateClocks(){
    const now=new Date();
    const text=iranClockFormatter?iranClockFormatter.format(now):now.toLocaleString('fa-IR',{timeZone:'Asia/Tehran'});
    document.querySelectorAll('[data-iran-clock]').forEach(el=>{el.textContent=text;el.setAttribute('datetime',now.toISOString());});
  }
  ...
  async function json(url,options={}){
    ...
    let body;
    try{body=await response.json();}catch(_){throw new Error(`HTTP ${response.status}: پاسخ JSON معتبر نیست`);}
    if(!response.ok||body?.ok===false)throw new Error(body?.message||body?.error||`HTTP ${response.status}`);
    return body;
  }
  function poll(fn,interval=5000,{immediate=true,visibleOnly=true}={}){
    let stopped=false,busy=false,timer=null;
    const tick=async()=>{
      if(stopped||busy)return;
      if(visibleOnly&&document.hidden)return;
      busy=true;try{await fn();}catch(e){console.warn('[Trade live]',e);}finally{busy=false;}
    };
    if(immediate)tick();
    timer=setInterval(tick,Math.max(1500,interval));
  ...
  document.addEventListener('DOMContentLoaded',()=>{updateThemeButton();updateClocks();});
  updateThemeButton();updateClocks();setInterval(updateClocks,1000);

  window.TradeUI={json,poll,escapeHtml,setText,formatNumber,formatBytes,iranFromPayload,toggleTheme};
})();
