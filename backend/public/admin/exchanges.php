    ...
    try{
        ...
        if($action==='bot_toggle'||$action==='live_toggle'){
            ...
            $message=match($source){
                ...
                'bit24'=>'API Key و کلید اختصاصی بیت۲۴ به‌صورت رمزنگاری‌شده ذخیره شدند.',
...
<section class="panel source-card"><div class="panel-head"><div><h2>بیت۲۴</h2><p class="market-role">API رسمی؛ API Key برای درخواست‌ها و کلید اختصاصی برای امضای HMAC-SHA256 ذخیره می‌شود. این اتصال در Trade فقط Market Data است.</p></div><span class="badge <?=sourceBadge($sourceStatus['bit24']??[])?>"><?=sourceText($sourceStatus['bit24']??[])?></span></div>
<form method="post" autocomplete="off"><input type="hidden" name="csrf" value="<?=$csrf?>"><input type="hidden" name="action" value="save_market_data"><input type="hidden" name="source" value="bit24"><div class="field-grid"><div class="field"><label>API Key بیت۲۴</label><input type="password" name="api_key" required autocomplete="new-password" spellcheck="false" style="direction:ltr"><span class="field-help">در هدر X-BIT24-APIKEY استفاده می‌شود.</span></div><div class="field"><label>کلید اختصاصی / Secret Key بیت۲۴</label><input type="password" name="secret" required autocomplete="new-password" spellcheck="false" style="direction:ltr"><span class="field-help">برای HMAC-SHA256 درخواست‌های امضاشده؛ رمزنگاری می‌شود و در UI نمایش داده نمی‌شود.</span></div></div><div class="actions"><button class="btn">ذخیره امن هر دو کلید</button><?php if($sourceStatus['bit24']['configured']??false):?><button class="btn danger" type="submit" name="action" value="delete_market_data" formnovalidate>حذف هر دو کلید</button><?php endif?></div></form></section>
