...
try{
    ...
    if($method==='POST'&&preg_match('#^/webhooks/tradingview/([A-Za-z0-9_-]{32,128})$#',$path,$m)){
        ...
        http_response_code(202);echo json_encode(['ok'=>true,'accepted'=>$result['accepted'],'duplicate'=>$result['duplicate'],'event_id'=>$result['event_id'],'time_iran'=>IranClock::nowPayload()],JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES|JSON_THROW_ON_ERROR);
        if(function_exists('fastcgi_finish_request')){fastcgi_finish_request();ignore_user_abort(true);if($tv->shouldInstantRecheck($result)){try{(new NobitexAutoTraderEngine())->run();}catch(Throwable $e){ErrorReporter::captureThrowable($e,'error','tradingview_instant_recheck');error_log('[Trade TradingView] instant recheck failed: '.$e->getMessage());}}}
        exit;
    }

    if($method==='GET'&&$path==='/api/health'){
        $db=true;try{$pdo=Database::connection();$pdo->query('SELECT 1');AppAccess::bootstrapLegacy($pdo);}catch(Throwable $e){$db=false;ErrorReporter::captureThrowable($e,'critical','health_database');}
        $bot=$db?(new BotController())->status():null;$tv=$db?(new TradingViewSignalService())->publicStatus():null;
        respond(['ok'=>$db,'service'=>'Trade','execution_mode'=>'live_only','capital_asset'=>'IRT/USDT','quote_priority'=>['IRT','USDT'],'version'=>Updater::currentVersion(),'api_contract'=>ReleaseContract::API_CONTRACT,'capabilities'=>ReleaseContract::CAPABILITIES,'app_url'=>(string)Config::get('app.url','https://rado-taxi.sbs'),'database'=>$db?'ok':'error','exchanges'=>$bot['exchanges']??[],'tradingview'=>$tv,'cron_health'=>$bot['cron_health']??null,'update_state'=>Updater::state(),'time_utc'=>gmdate(DATE_ATOM),'time_iran'=>IranClock::nowPayload()],$db?200:503);
    }
    ...
    if($method==='POST'&&$path==='/api/pair'){
        $body=jsonBody();$code=trim((string)($body['code']??''));if($code==='')throw new InvalidArgumentException('کد اتصال لازم است.');$paired=AppAccess::consumePairing(Database::connection(),$code);respond(['ok'=>true,'data'=>['token'=>$paired['token'],'token_id'=>$paired['token_id'],'label'=>$paired['label'],'server_url'=>(string)Config::get('app.url','https://rado-taxi.sbs'),'time_iran'=>IranClock::nowPayload()]]);
    }
    ...
    if($method==='GET'&&$path==='/api/status'){
        ...
        respond(['ok'=>true,'data'=>[
            ...
            'global_portfolio'=>globalPortfolioSnapshot(),'notification_unread_count'=>$notifications->unreadCount($pdo),'time_iran'=>IranClock::nowPayload(),
        ]]);
    }
    ...
    if($method==='DELETE'&&preg_match('#^/api/orders/([^/]+)$#',$path,$m)){$exchange=exchangeName();$data=$exchange==='nobitex'?(new NobitexOrderService())->cancel($m[1]):(new OrderService())->cancel($m[1]);respond(['ok'=>true,'exchange'=>$exchange,'data'=>$data,'time_iran'=>IranClock::nowPayload()]);}
