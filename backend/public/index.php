...
try{
    ...
    if($method==='GET'&&$path==='/api/markets'){
        ...
        if($exchange==='nobitex'){$client=(new NobitexOrderService())->client();respond(['ok'=>true,'exchange'=>'nobitex','data'=>$client->allOrderBooks()]);}
        $service=new OrderService();$client=$service->client();$query=$_GET;unset($query['exchange']);respond(['ok'=>true,'exchange'=>'bitpin','data'=>$client->markets($query)]);
    }
    if($method==='GET'&&$path==='/api/wallets'){
        $exchange=exchangeName();
        if($exchange==='nobitex'){$client=(new NobitexOrderService())->client();respond(['ok'=>true,'exchange'=>'nobitex','data'=>$client->wallets()]);}
        $service=new OrderService();$client=$service->client();$query=$_GET;unset($query['exchange']);$data=$client->wallets($query);$service->syncTokens($client);respond(['ok'=>true,'exchange'=>'bitpin','data'=>$data]);
    }
    if($method==='GET'&&$path==='/api/orders'){
        $exchange=exchangeName();
        if($exchange==='nobitex'){$client=(new NobitexOrderService())->client();$query=$_GET;unset($query['exchange']);respond(['ok'=>true,'exchange'=>'nobitex','data'=>$client->orders($query)]);}
        $service=new OrderService();$client=$service->client();$query=$_GET;unset($query['exchange']);$data=$client->orders($query);$service->syncTokens($client);respond(['ok'=>true,'exchange'=>'bitpin','data'=>$data]);
    }
    if($method==='POST'&&$path==='/api/orders'){
        $body=jsonBody();$exchange=exchangeName($body);unset($body['exchange']);
        $data=$exchange==='nobitex'?(new NobitexOrderService())->create($body,'android_or_api'):(new OrderService())->create($body,'android_or_api');
        respond(['ok'=>true,'exchange'=>$exchange,'data'=>$data],201);
    }
    if($method==='DELETE'&&preg_match('#^/api/orders/([^/]+)$#',$path,$m)){
        $exchange=exchangeName();$data=$exchange==='nobitex'?(new NobitexOrderService())->cancel($m[1]):(new OrderService())->cancel($m[1]);respond(['ok'=>true,'exchange'=>$exchange,'data'=>$data]);
    }
