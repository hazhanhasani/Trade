...
try{
    ...
    if($method==='POST'&&$path==='/api/notifications/read'){$body=jsonBody();$center=new TradeNotificationCenter();$ids=is_array($body['ids']??null)?$body['ids']:[];if($ids!==[]){$marked=$center->markReadMany($ids);respond(['ok'=>true,'data'=>['marked'=>$marked,'unread_count'=>$center->unreadCount()]]);} $all=boolValue($body['all']??false);$id=isset($body['id'])?(int)$body['id']:null;if(!$all&&($id===null||$id<=0))throw new InvalidArgumentException('ids, id or all=true is required.');$center->markRead($all?null:$id);respond(['ok'=>true,'data'=>['marked'=>1,'unread_count'=>$center->unreadCount()]]);}
