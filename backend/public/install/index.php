...
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    $bitpinKey=trim((string)($_POST['bitpin_key']??''));$bitpinSecret=trim((string)($_POST['bitpin_secret']??''));
    ...
    $bitpinLive=isset($_POST['bitpin_live']);$bitpinBot=isset($_POST['bitpin_bot']);
    ...
    if(($bitpinLive||$bitpinBot)&&($bitpinKey===''||$bitpinSecret===''))$errors[]='Bitpin Live/Bot requires Bitpin credentials.';
    if($bitpinBot&&!$bitpinLive)$errors[]='Bitpin automatic trading requires Bitpin Live execution.';
    ...
    if($errors===[]){
        try{
            ...
            $config=[
                'app'=>['installed'=>true,'url'=>'https://rado-taxi.sbs','mode'=>$bitpinLive?'live':'live_disabled','encryption_key'=>$encryptionKey,'api_token_hash'=>hash('sha256',$apiToken)],
                'database'=>['host'=>$dbHost,'port'=>$dbPort,'name'=>$dbName,'user'=>$dbUser,'password'=>$dbPass],
                'bitpin'=>['base_url'=>'https://api.bitpin.market/api/v1','timeout'=>12,'endpoints'=>['authenticate'=>'/usr/authenticate/','refresh'=>'/usr/refresh_token/','markets'=>'/mkt/markets/','wallets'=>'/wlt/wallets/','orders'=>'/odr/orders/','tickers'=>'/mkt/tickers/','orderbook'=>'/mth/orderbook/{symbol}/','matches'=>'/mth/matches/{symbol}/']],
                'nobitex'=>['base_url'=>'https://apiv2.nobitex.ir','public_base_url'=>'https://api.nobitex.ir','timeout'=>12],
                'trading'=>['enabled'=>$bitpinLive,'capital_asset'=>'GRAM','legacy_asset_alias'=>'TON','max_order_value'=>$maxOrderValue,'max_orders_per_hour'=>$maxOrdersPerHour],
                'updates'=>['auto_backend'=>true,'check_interval_seconds'=>300,'manifest_url'=>'https://github.com/hazhanhasani/Trade/releases/download/trade-latest/latest.json'],
            ];
            ...
            if($bitpinKey!==''&&$bitpinSecret!==''){$stmt=$pdo->prepare("INSERT INTO exchange_credentials (exchange_name,api_key_enc,secret_key_enc,created_at,updated_at) VALUES ('bitpin',:k,:s,UTC_TIMESTAMP(),UTC_TIMESTAMP())");$stmt->execute([':k'=>$encrypt($bitpinKey),':s'=>$encrypt($bitpinSecret)]);}
            ...
            $settings=['kill_switch'=>'0','live_trading_enabled'=>$bitpinLive?'1':'0','autotrade_bitpin_enabled'=>$bitpinBot?'1':'0','live_trading_nobitex_enabled'=>'0','autotrade_nobitex_enabled'=>'0','autotrade_schema_version'=>'1'];
            $stmt=$pdo->prepare("INSERT INTO settings (key_name,value_text,updated_at) VALUES (:k,:v,UTC_TIMESTAMP()) ON DUPLICATE KEY UPDATE value_text=VALUES(value_text),updated_at=UTC_TIMESTAMP()");foreach($settings as$k=>$v)$stmt->execute([':k'=>$k,':v'=>$v]);
            $stmt=$pdo->prepare("INSERT INTO autotrade_settings (id,enabled,quote_asset,risk_profile,position_percent,max_position_percent,stop_loss_percent,take_profit_percent,daily_loss_limit_percent,min_signal_score,cooldown_minutes,updated_at) VALUES (1,:enabled,'USDT','balanced',5,10,3,6,5,60,15,UTC_TIMESTAMP()) ON DUPLICATE KEY UPDATE enabled=VALUES(enabled),updated_at=UTC_TIMESTAMP()");$stmt->execute([':enabled'=>$bitpinBot?1:0]);
            file_put_contents($lock,gmdate(DATE_ATOM)."\n",LOCK_EX);@chmod($lock,0600);$success=true;
        }catch(Throwable $e){$errors[]=$e->getMessage();}
    }
}
?><!doctype html><html lang="fa" dir="rtl"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>نصب Trade</title><style>body{font-family:Tahoma,Arial;background:#f4f7fb;color:#172033;margin:0}.wrap{max-width:820px;margin:30px auto;padding:16px}.card{background:#fff;border:1px solid #e4e9f2;border-radius:20px;padding:24px}.grid{display:grid;grid-template-columns:1fr 1fr;gap:12px}.full{grid-column:1/-1}label{display:block;font-size:13px;margin:8px 0 5px}input{width:100%;box-sizing:border-box;padding:12px;border:1px solid #ccd5e3;border-radius:10px}button{width:100%;padding:13px;border:0;border-radius:12px;background:#1769ff;color:#fff;font-weight:700;margin-top:18px}.ok,.info,.warn,.err{padding:12px;border-radius:12px;margin:10px 0}.ok{background:#ecfff2}.info{background:#eef5ff}.warn{background:#fff8e7}.err{background:#fff1f1;color:#a11}.token{direction:ltr;word-break:break-all;background:#f2f5fa;padding:12px;border-radius:10px}@media(max-width:650px){.grid{grid-template-columns:1fr}.wrap{margin:5px auto}.full{grid-column:auto}}</style></head><body><div class="wrap"><div class="card"><h1>نصب Trade</h1><div class="info">دارایی داخلی: <b>GRAM</b>؛ نام TON برای سازگاری با نماد صرافی‌ها حفظ می‌شود.</div><div class="warn">Bitpin و Nobitex مستقل مدیریت می‌شوند. Nobitex در نصب اولیه حتی با وجود کلید API به‌صورت Bot OFF / Live OFF باقی می‌ماند تا از پنل صرافی‌ها صریحاً فعال شود.</div>
...
<?php if($success):?><div class="ok"><b>نصب موفق بود.</b><p>پنل: <code>/admin/</code> — صرافی‌ها: <code>/admin/exchanges.php</code> — ربات‌ها: <code>/admin/bot/</code></p><p>توکن اتصال اپ:</p><div class="token"><?=htmlspecialchars((string)$apiToken)?></div><p>Cron را Every Minute روی <code>cron/tick.php</code> تنظیم کن.</p></div><?php else:?><form method="post" autocomplete="off"><div class="grid"><div><label>DB Host</label><input name="db_host" value="localhost" required></div><div><label>DB Port</label><input name="db_port" value="3306" required></div><div><label>Database</label><input name="db_name" required></div><div><label>DB User</label><input name="db_user" required></div><div class="full"><label>DB Password</label><input type="password" name="db_pass"></div><div><label>Admin username</label><input name="admin_user" value="admin" required></div><div><label>Admin password (حداقل ۱۲ کاراکتر)</label><input type="password" name="admin_pass" minlength="12" required></div>
<div class="full"><h3>Bitpin</h3></div><div class="full"><label>Bitpin API Key</label><input name="bitpin_key" autocomplete="off"></div><div class="full"><label>Bitpin Secret Key</label><input type="password" name="bitpin_secret" autocomplete="new-password"></div><div class="full"><label><input style="width:auto" type="checkbox" name="bitpin_live"> ارسال سفارش واقعی Bitpin</label></div><div class="full"><label><input style="width:auto" type="checkbox" name="bitpin_bot"> ربات خودکار Bitpin</label></div>
<div class="full"><h3>Nobitex</h3><div class="info">API Key نوبیتکس با مجوزهای READ + TRADE کافی است؛ WITHDRAW لازم نیست.</div></div><div class="full"><label>Nobitex Public Key</label><input name="nobitex_public_key" autocomplete="off"></div><div class="full"><label>Nobitex Private Key</label><input type="password" name="nobitex_private_key" autocomplete="new-password"></div>
<div><label>حداکثر ارزش هر سفارش (۰ = بدون سقف)</label><input type="number" min="0" step="any" name="max_order_value" value="0"></div><div><label>حداکثر سفارش در ساعت</label><input type="number" min="1" max="100" name="max_orders_per_hour" value="10"></div></div><button type="submit">نصب Production</button></form><?php endif?></div></div></body></html>
