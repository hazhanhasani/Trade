...
if (is_file($lock)) {
    ...
    exit('Trade is already installed. Remove storage/install.lock manually only if you intentionally want to reinstall.');
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $tradingEnabled = isset($_POST['trading_enabled']);
    ...
    if ($errors === []) {
        try {
            ...
            $stmt->execute([':u' => $adminUser, ':p' => password_hash($adminPass, PASSWORD_ARGON2ID)]);
...
<?php if ($success): ?>
<div class="ok"><strong>نصب با موفقیت انجام شد.</strong><p>توکن API اپلیکیشن فقط همین یک‌بار نمایش داده می‌شود. آن را در جای امن نگه دارید:</p><div class="token"><?= htmlspecialchars((string)$apiToken) ?></div><p>حالا Cron را طبق README تنظیم کنید و پوشه <code>/install</code> را نیز می‌توانید از دسترس وب خارج کنید.</p></div>
<?php else: ?>
...
<div class="full"><label>Bitpin API Key (اختیاری در نصب)</label><input name="bitpin_key" autocomplete="off"></div>
<div class="full"><label>Bitpin Secret Key (اختیاری در نصب)</label><input type="password" name="bitpin_secret" autocomplete="new-password"></div>
<div><label>حداکثر ارزش هر سفارش (۰ = بدون سقف عددی برنامه)</label><input type="number" step="any" min="0" name="max_order_value" value="0"></div>
<div style="display:flex;align-items:end;padding-bottom:10px"><label><input style="width:auto" type="checkbox" name="trading_enabled"> فعال‌سازی اجرای سفارش روی سرور</label></div>
</div><button type="submit">نصب</button></form>
<?php endif; ?>
</div></div></body></html>
