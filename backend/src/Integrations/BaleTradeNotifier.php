...
final class BaleTradeNotifier
{
    ...
    private const RECOVERY_LOOKBACK_SECONDS = 86400;
    private const MAX_RECOVERY_LOOKBACK_SECONDS = 604800;
    ...
    public function status(?PDO $pdo = null): array
    {
        ...
        if(is_array($last)){
            foreach(['last_attempt_at','sent_at']as$k)if(!empty($last[$k]))$last[$k.'_iran']=IranClock::fromUtc((string)$last[$k]);
        }
        return[
            'enabled'=>$enabled,
            ...
            'recovery_lookback_seconds'=>self::RECOVERY_LOOKBACK_SECONDS,
    ...
    public function saveAndTest(string $tokenInput,string $chatId,bool $enabled=true,?PDO $pdo=null):array
    {
        ...
        return[
            'ok'=>true,'enabled'=>$enabled,'chat_id'=>$chatId,'bot'=>$bot['result']??null,
            'test_message_id'=>$test['result']['message_id']??null,'time_iran'=>IranClock::nowPayload(),
        ];
    }
    ...
    public function syncConfirmedTrades(int $limit=50,?PDO $pdo=null):array
    {
        ...
        if(!$this->boolSetting($pdo,self::ENABLED_KEY))return['buys_queued'=>0,'sells_queued'=>0,'since'=>null];
        if(!(bool)($this->status($pdo)['configured']??false))return['buys_queued'=>0,'sells_queued'=>0,'since'=>null];

        $since=$this->effectiveSyncSince($pdo);
        ...
        foreach($stmt->fetchAll()as$row){
            ...
            $this->queueConfirmedTrade($eventKey,'buy',[
                ...
                'strategy_key'=>$meta['strategy_key']??null,'market_regime'=>$meta['market_regime']??null,
                'tradable_net_edge_percent'=>$meta['tradable_net_edge_percent']??null,
                'time_iran'=>IranClock::formatUtc((string)$row['opened_at']),
            ],$pdo,true);
            $buysQueued++;
        }
        ...
        return['buys_queued'=>$buysQueued,'sells_queued'=>$sellsQueued,'since'=>$since];
    }
    ...
    public function queueConfirmedTrade(string $eventKey,string $tradeType,array $trade,?PDO $pdo=null,bool $attemptImmediate=true):void
    {
        ...
        $stmt=$pdo->prepare("INSERT INTO bale_trade_deliveries (event_key,trade_type,payload_json,status,attempts,created_at,updated_at) VALUES (:event_key,:trade_type,:payload,'pending',0,UTC_TIMESTAMP(),UTC_TIMESTAMP()) ON DUPLICATE KEY UPDATE event_key=event_key");
        $stmt->execute([':event_key'=>$eventKey,':trade_type'=>$tradeType,':payload'=>$payload]);
        if($attemptImmediate){
            // Delivery failures are recorded by deliverEvent. Never let one Bale
            // network/API failure abort discovery/queueing of the remaining fills.
            try{$this->deliverEvent($eventKey,$pdo);}catch(\Throwable){}
        }
    }
    ...
    public function flushPending(int $limit=10,?PDO $pdo=null):array
    {
        ...
        $rows=$pdo->query("SELECT event_key FROM bale_trade_deliveries WHERE sent_at IS NULL AND (status IN ('pending','failed') OR (status='sending' AND last_attempt_at < (UTC_TIMESTAMP() - INTERVAL 5 MINUTE))) AND (last_attempt_at IS NULL OR last_attempt_at < (UTC_TIMESTAMP() - INTERVAL 15 SECOND)) ORDER BY id ASC LIMIT {$limit}")->fetchAll();
        $out=['attempted'=>0,'sent'=>0,'failed'=>0];
        foreach($rows as$row){
            $out['attempted']++;
            try{if($this->deliverEvent((string)$row['event_key'],$pdo))$out['sent']++;else$out['failed']++;}
    ...
    public static function formatTradeMessage(string $tradeType,array $trade):string
    {
        ...
        if($tradeType==='buy'){
            $lines=['🟢 خرید انجام شد','صرافی: نوبیتکس','بازار: '.($asset!==''?$asset:$symbol).' / '.$displayUnit,
            ...
            if(isset($trade['tradable_net_edge_percent'])&&is_numeric($trade['tradable_net_edge_percent']))$lines[]='سود مورد انتظار پس از هزینه‌ها: '.self::formatNumber((float)$trade['tradable_net_edge_percent'],3).'٪';
            $lines[]='زمان ایران (شمسی): '.$time;
            return implode("\n",$lines);
        }
        ...
        $lines=['🔴 فروش انجام شد','صرافی: نوبیتکس','بازار: '.($asset!==''?$asset:$symbol).' / '.$displayUnit,
            'مقدار: '.self::formatNumber($amount,8).($asset!==''?' '.$asset:''),
    ...
    private function effectiveSyncSince(PDO $pdo):string
    {
        $now=time();
        $configured=trim((string)($this->setting($pdo,self::START_KEY)??''));
        if($configured===''){
            $configured=gmdate('Y-m-d H:i:s',$now);
            $this->writeSetting($pdo,self::START_KEY,$configured);
        }
        $configuredTs=strtotime($configured.' UTC');
        if($configuredTs===false)$configuredTs=$now;
        $recoveryTs=$now-self::RECOVERY_LOOKBACK_SECONDS;
        $oldestAllowed=$now-self::MAX_RECOVERY_LOOKBACK_SECONDS;
        return gmdate('Y-m-d H:i:s',max($oldestAllowed,min($configuredTs,$recoveryTs)));
    }
    ...
    private function call(string $token,string $method,array $payload=[]):array
    {
        ...
        curl_setopt_array($ch,[
            ...
            CURLOPT_HTTPHEADER=>['Content-Type: application/json','Accept: application/json'],CURLOPT_POSTFIELDS=>$body,
        ]);
        try{
            $raw=curl_exec($ch);
    ...
    {
        $this->ensureSchema($pdo);
        $stmt=$pdo->prepare("UPDATE bale_trade_deliveries SET status='sending',attempts=attempts+1,last_attempt_at=UTC_TIMESTAMP(),updated_at=UTC_TIMESTAMP() WHERE event_key=:event_key AND sent_at IS NULL AND (status IN ('pending','failed') OR (status='sending' AND last_attempt_at < (UTC_TIMESTAMP() - INTERVAL 5 MINUTE)))");
