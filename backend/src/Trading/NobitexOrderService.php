...
final class NobitexOrderService
{
    ...
    {
        $stmt=Database::connection()->prepare("SELECT EXISTS(SELECT 1 FROM exchange_credentials WHERE exchange_name='nobitex')");$stmt->execute();return(bool)$stmt->fetchColumn();
    }

    public function saveCredentials(string $publicKey,string $privateKey):void
    {
        $publicKey=trim($publicKey);$privateKey=trim($privateKey);
        ...
        $test=new NobitexClient(['base_url'=>Config::get('nobitex.base_url','https://apiv2.nobitex.ir'),'public_key'=>$publicKey,'private_key'=>$privateKey,'timeout'=>(int)Config::get('nobitex.timeout',12)]);$test->test();
        ...
        $stmt->execute([':pub'=>Crypto::encrypt($publicKey,$enc),':priv'=>Crypto::encrypt($privateKey,$enc)]);$this->audit('nobitex.credentials_saved');
    }

    public function deleteCredentials():void
    {
        Database::connection()->exec("DELETE FROM exchange_credentials WHERE exchange_name='nobitex'");$this->setLiveEnabled(false);NobitexSchema::setBotEnabled(false);$this->audit('nobitex.credentials_deleted');
    }

    public function liveEnabled():bool
    {
        $stmt=Database::connection()->prepare("SELECT value_text FROM settings WHERE key_name='live_trading_nobitex_enabled' LIMIT 1");$stmt->execute();return in_array(strtolower(trim((string)($stmt->fetchColumn()?:'0'))),['1','true','yes','on'],true);
    }
    ...
    public function setLiveEnabled(bool $enabled):void
    {
        ...
        $stmt=Database::connection()->prepare("INSERT INTO settings (key_name,value_text,updated_at) VALUES ('live_trading_nobitex_enabled',:v,UTC_TIMESTAMP()) ON DUPLICATE KEY UPDATE value_text=VALUES(value_text),updated_at=UTC_TIMESTAMP()");$stmt->execute([':v'=>$enabled?'1':'0']);$this->audit($enabled?'nobitex.live_enabled':'nobitex.live_disabled');
    }
    ...
    public function create(array $input,string $source='api'):array
    {
        ...
        [$base,$quote]=$this->parseSymbol($symbol);if($base===''||$quote==='')throw new \InvalidArgumentException('Nobitex symbol must look like TONUSDT or TONIRT.');
        ...
        $side=strtolower(trim((string)($input['side']??$input['type']??'')));if(!in_array($side,['buy','sell'],true))throw new \InvalidArgumentException('side/type must be buy or sell.');
        ...
        $payload=['type'=>$side,'srcCurrency'=>strtolower($base),'dstCurrency'=>strtolower($quote==='IRT'?'rls':$quote),'amount'=>$this->num($amount),'clientOrderId'=>$clientOrderId];
        if($mode==='oco'){
            $payload['mode']='oco';$payload['price']=$this->num((float)$price);$payload['stopPrice']=$this->num($this->positive($input['stopPrice']??$input['price_stop']??null,'stopPrice'));$payload['stopLimitPrice']=$this->num($this->positive($input['stopLimitPrice']??$input['price_limit_oco']??$input['price_limit']??null,'stopLimitPrice'));
        }else{
        ...
        try{
            ...
            $stmt=$pdo->prepare("UPDATE orders SET status='failed',error_text=:error,updated_at=UTC_TIMESTAMP() WHERE local_id=:local");$stmt->execute([':error'=>mb_substr($e->getMessage(),0,1000),':local'=>$localId]);$this->audit('nobitex.order_failed',['local_id'=>$localId,'error'=>$e->getMessage(),'source'=>$source]);
            try{(new TradeNotificationCenter())->emit('order-failed:'.$localId,'execution','critical','ارسال سفارش ناموفق بود',$symbol.' • '.mb_substr($e->getMessage(),0,180),['local_id'=>$localId,'symbol'=>$symbol,'side'=>$side]);}catch(\Throwable){}
            throw$e;
        }
    }
    ...
    {
        $this->assertEnabled();$orderId=trim($orderId);$client=$this->client();$response=ctype_digit($orderId)?$client->cancelOrder($orderId):$client->cancelOrder(null,$orderId);$stmt=Database::connection()->prepare("UPDATE orders SET status='cancelled',updated_at=UTC_TIMESTAMP() WHERE exchange_name='nobitex' AND (exchange_order_id=:id OR identifier=:id)");$stmt->execute([':id'=>$orderId]);$this->audit('nobitex.order_cancelled',['order_id'=>$orderId]);return$response;
    }
    public function normalizedOrder(array $response):array{return$this->firstOrder($response);}
    ...
    private function parseSymbol(string $symbol):array{foreach(['USDT','IRT']as$quote){if(str_ends_with($symbol,$quote)&&strlen($symbol)>strlen($quote)){$base=substr($symbol,0,-strlen($quote));if($base==='GRAM'||$base==='TONCOIN')$base='TON';return[$base,$quote];}}return['',''];}
    private function firstOrder(array $response):array{if(is_array($response['order']??null))return$response['order'];if(is_array($response['orders']??null)&&is_array($response['orders'][0]??null))return$response['orders'][0];return$response;}
    private function positive(mixed $value,string $name):float{if(!is_numeric($value)||!is_finite((float)$value)||(float)$value<=0)throw new \InvalidArgumentException($name.' must be positive.');return(float)$value;}
    private function num(float $value):string{return rtrim(rtrim(sprintf('%.12F',$value),'0'),'.');}
    private function audit(string $event,array $context=[]):void{$stmt=Database::connection()->prepare('INSERT INTO audit_logs (event_name,context_json,created_at) VALUES (:event,:context,UTC_TIMESTAMP())');$stmt->execute([':event'=>$event,':context'=>$context===[]?null:json_encode($context,JSON_THROW_ON_ERROR|JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE)]);}
}
