...
final class NobitexInternalSignalEngine
{
    ...
    public function analyze(array $market, array $minutePrices, int $legacyThreshold = 60): array
    {
        ...
        $rawGross = ($momentum * 0.52) + ($trend * 0.34) + ($macdPressure * 0.75) + $consistencyBias + $flowBias - $rsiPenalty;

        $regimePenalty = 0.0;
        if ($m1 < 0.0 && $m5 < 0.0) $regimePenalty += min(0.25, (abs($m1) * 0.08) + (abs($m5) * 0.03));
        if ($m5 < 0.0 && $m15 < 0.0) $regimePenalty += min(0.30, (abs($m5) * 0.03) + (abs($m15) * 0.02));
        if ($trend < 0.0) $regimePenalty += min(0.20, abs($trend) * 0.08);
        ...
        $dynamicSpreadLimit = $this->clamp(self::MIN_DYNAMIC_SPREAD_PERCENT + ($volatility * 0.65), self::MIN_DYNAMIC_SPREAD_PERCENT, self::MAX_EXECUTABLE_SPREAD_PERCENT);
        ...
        $result = [
            ...
            'execution_quality'=>[
                ...
                'liquidity_multiple'=>round($liquidityMultiple, 4),'minimum_liquidity_multiple'=>self::MIN_LIQUIDITY_MULTIPLE,
                'target_liquidity_multiple'=>self::TARGET_LIQUIDITY_MULTIPLE,'liquidity_coverage'=>round($liquidityCoverage, 4),
                'dynamic_max_spread_percent'=>round($dynamicSpreadLimit, 4),'positive_timeframe_count'=>$agreementCount,
                'orderbook_imbalance'=>round($imbalance, 4),
            ],
            'cost_model'=>[
                'quote_asset'=>strtoupper((string) ($market['quote_asset'] ?? 'IRT')),
                'base_roundtrip_taker_fee_percent'=>round($baseFee, 4),'spread_cost_percent'=>round($spreadCost, 4),
                ...
                'adverse_flow_reserve_percent'=>round($adverseFlowReserve, 4),'adaptive_forecast_buffer_percent'=>round($edgeBuffer, 4),
                'forecast_buffer_model'=>'uncertainty_only_v2','forecast_buffer_base_percent'=>$forecastBuffer['base_percent'],
            ...
            'indicators'=>[
                'momentum_blend_percent'=>round($momentum,4),'trend_blend_percent'=>round($trend,4),
                'macd_pressure_percent'=>round($macdPressure,4),'trend_consistency'=>round($trendConsistency,4),
                'volatility_percent'=>round($volatility,4),'orderbook_imbalance'=>round($imbalance,4),'spread_percent'=>round($spread,6),
                'rsi_penalty_percent'=>round($rsiPenalty,4),'regime_risk_penalty_percent'=>round($regimePenalty,4),
                'timeframe_disagreement_penalty_percent'=>round($disagreementPenalty,4),'exhaustion_penalty_percent'=>round($exhaustionPenalty,4),
            ],
            'tradingview'=>['enabled'=>false,'used'=>false,'required'=>false],
        ];

        return NobitexAdaptivePolicyLearner::applyToSignal($result, $market);
    }

    /**
     * Residual model/forecast uncertainty after explicit execution costs have
     * already been deducted from gross edge. Kept public for deterministic tests.
     */
    public static function forecastUncertaintyBuffer(
        ...
        $base = 0.12;
        $friction = min(0.12, ($spreadCost * 0.10) + ($liquidityReserve * 0.10) + ($adverseFlowReserve * 0.10));
        ...
        $total = max(self::MIN_EDGE_BUFFER_PERCENT,
            min(self::MAX_EDGE_BUFFER_PERCENT, $base + $friction + $volatilityUncertainty + $disagreement + $exhaustion));
        return [
            'model'=>'uncertainty_only_v2','base_percent'=>round($base, 4),'friction_uncertainty_percent'=>round($friction, 4),
            'volatility_uncertainty_percent'=>round($volatilityUncertainty, 4),'disagreement_uncertainty_percent'=>round($disagreement, 4),
            'exhaustion_uncertainty_percent'=>round($exhaustion, 4),'total_percent'=>round($total, 4),
        ];
    }
    ...
    private function baseRoundtripFeePercent(array $market): float
    {
        ...
        return $quote === 'USDT' ? self::USDT_ROUNDTRIP_TAKER_FEE_PERCENT : self::IRT_ROUNDTRIP_TAKER_FEE_PERCENT;
    }
    ...
    private function notReady(array $minute, array $five, array $fifteen, string $reason): array
    {
        return [
            ...
            'estimated_exit_cost_percent'=>0.0,'expected_net_edge_percent'=>0.0,'required_edge_buffer_percent'=>0.0,
            'tradable_net_edge_percent'=>0.0,'minimum_net_edge_percent'=>0.0,
