...
final class RiskManager
{
    ...
    public function entryDecision(
        ...
        if ($portfolioValue <= 0 || $orderValue <= 0) {
        ...
        if ($lastTradeAt) {
            ...
            if ($last !== false) {
                ...
                if (time() < $next) {
                    return ['allowed' => false, 'reason' => 'cooldown_active', 'cooldown_until' => gmdate(DATE_ATOM, $next)];
                }
            }
        }
