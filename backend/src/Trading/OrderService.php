...
final class OrderService
{
    ...
    public function create(array $input, string $source = 'api'): array
    {
        ...
        if (!$market || $market < 1) throw new \InvalidArgumentException('market must be a positive Bitpin market id.');
        ...
        if (!in_array($type, ['buy','sell'], true)) throw new \InvalidArgumentException('type must be buy or sell.');
        if (!in_array($mode, ['limit','market','stop_limit','oco'], true)) throw new \InvalidArgumentException('Unsupported order mode.');
        ...
        foreach (['price_limit','price_stop','price_limit_oco','amount2'] as $field) {
            if (isset($input[$field]) && $input[$field] !== '') $payload[$field] = $this->num($this->positive($input[$field], $field));
        }
        ...
        $stmt->execute([
            ':local'=>$localId, ':identifier'=>$identifier, ':market'=>(string)$market, ':side'=>$type,
            ':mode'=>$mode, ':amount'=>$payload['amount1'], ':price'=>$payload['price'], ':source'=>$source,
        ...
        try {
            ...
            $stmt = $pdo->prepare("UPDATE orders SET exchange_order_id=:eid,status='submitted',response_json=:response,updated_at=UTC_TIMESTAMP() WHERE local_id=:local");
            $stmt->execute([':eid'=>$exchangeId !== '' ? $exchangeId : null, ':response'=>json_encode($response, JSON_THROW_ON_ERROR|JSON_UNESCAPED_SLASHES), ':local'=>$localId]);
            $this->audit('order.live_submitted', ['local_id'=>$localId,'exchange_order_id'=>$exchangeId,'source'=>$source]);
            ...
            $this->audit('order.live_failed', ['local_id'=>$localId,'error'=>$e->getMessage()]);
            throw $e;
        }
    }
    ...
    private function assertAllowed(array $order): void
    {
        ...
        $count = (int) $pdo->query("SELECT COUNT(*) FROM orders WHERE created_at >= (UTC_TIMESTAMP() - INTERVAL 1 HOUR) AND status IN ('submitting','submitted')")->fetchColumn();
        ...
        if ($maxValue > 0 && isset($order['amount1'],$order['price']) && ((float)$order['amount1'] * (float)$order['price']) > $maxValue) {
    ...
    {
        if (!(bool) Config::get('trading.enabled', false)) throw new \RuntimeException('Live trading is disabled in server configuration.');
    }
    ...
    private function audit(string $event, array $context): void
    {
        ...
        $stmt->execute([':event'=>$event, ':context'=>json_encode($context, JSON_THROW_ON_ERROR|JSON_UNESCAPED_SLASHES)]);
    }
}
