 ...
 * It runs before the generic pending watchdog, so stale marketable limits are
 * cancelled/replaced once while the original hard slippage boundary remains.
 */
final class NobitexExecutionRepriceService
{
    ...
    public function reconcile(?PDO $pdo=null):array
    {
        ...
        foreach($rows as $p){
            $positionId=(int)$p['id'];$meta=$this->repriceMeta($pdo,$positionId);
            ...
            try{$response=$remoteId!==''?$client->orderStatus($remoteId):$client->orderStatus(null,$identifier);$remote=$orders->normalizedOrder($response);}catch(\Throwable){$remote=[];}
            $state=strtolower(trim((string)($remote['status']??'')));$filled=$this->fillAmount($remote,0.0);
            if($filled>0){$results[]=['position_id'=>$positionId,'status'=>'partial_after_cancel'];continue;}
            if($state!==''&&!in_array($state,['canceled','cancelled','rejected','failed'],true)){$results[]=['position_id'=>$positionId,'status'=>'cancel_pending'];continue;}
            ...
            try{
                ...
                $stmt=$pdo->prepare("UPDATE nobitex_autotrade_positions SET entry_identifier=:i,entry_order_local_id=:l,entry_exchange_order_id=:x,entry_price=:price,created_at=UTC_TIMESTAMP(),updated_at=UTC_TIMESTAMP() WHERE id=:id AND status='pending_open'");
            ...
            }catch(\Throwable $e){$this->failPosition($pdo,$positionId);$results[]=['position_id'=>$positionId,'status'=>'replace_failed','error'=>mb_substr($e->getMessage(),0,180)];}
        }
        return['status'=>'processed','checked'=>count($rows),'results'=>$results];
    }

    public function registerPlan(PDO $pdo,int $positionId,array $plan):void
    {
        $this->ensure($pdo);$stmt=$pdo->prepare("INSERT INTO nobitex_execution_reprices (position_id,reprice_count,max_reprices,hard_price_limit,created_at,updated_at) VALUES (:id,0,:max,:hard,UTC_TIMESTAMP(),UTC_TIMESTAMP()) ON DUPLICATE KEY UPDATE max_reprices=VALUES(max_reprices),hard_price_limit=VALUES(hard_price_limit),updated_at=UTC_TIMESTAMP()");$stmt->execute([':id'=>$positionId,':max'=>max(0,min(2,(int)($plan['max_reprices']??0))),':hard'=>(float)($plan['hard_price_limit']??0)]);
    }

    private function repriceMeta(PDO $pdo,int $id):array{$s=$pdo->prepare('SELECT reprice_count,max_reprices,hard_price_limit FROM nobitex_execution_reprices WHERE position_id=:id LIMIT 1');$s->execute([':id'=>$id]);$r=$s->fetch();return$r?:['reprice_count'=>0,'max_reprices'=>0,'hard_price_limit'=>0];}
    private function failPosition(PDO $pdo,int $id):void{$pdo->prepare("UPDATE nobitex_autotrade_positions SET status='failed',updated_at=UTC_TIMESTAMP() WHERE id=:id AND status='pending_open'")->execute([':id'=>$id]);}
