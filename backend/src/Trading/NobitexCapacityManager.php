...
/**
 * Hard/adaptive live-position capacity enforcement.
 *
...
final class NobitexCapacityManager
{
    ...
    public function enforce(?PDO $pdo = null): array
    {
        ...
        foreach ($positions as $position) {
            try {
                ...
                $this->event($pdo,'warning','nobitex.capacity.position_scan_failed',['position_id'=>$position['id'] ?? null,'symbol'=>$position['symbol'] ?? null,'error'=>mb_substr($e->getMessage(),0,200)]);
                continue;
            }
            ...
            $netPct = is_numeric($position['unrealized_net_pnl_percent'] ?? null) ? (float)$position['unrealized_net_pnl_percent'] : $grossPct-$exitCost;
            $indicators = is_array($signal['indicators'] ?? null) ? $signal['indicators'] : [];
            $position['_market'] = $market;
            $position['forward_edge_percent'] = self::signalEdge($signal);
            $position['unrealized_net_pnl_percent'] = $netPct;
            $position['estimated_exit_cost_percent'] = $exitCost;
            $position['spread_percent'] = max(0.0,(float)($market['spread_percent'] ?? 0.0));
            $position['depth_quote'] = max(0.0,(float)($market['depth_quote'] ?? 0.0));
            $position['volatility_percent'] = max(0.0,(float)($indicators['volatility_percent'] ?? 0.0));
        ...
        if (($attempt['active'] ?? false) === true) return $this->result('waiting_reconcile','existing_capacity_exit_inflight',['position_id'=>(int)$victim['id'],'identifier'=>$attempt['identifier'] ?? null]);
        ...
        $stmt = $pdo->prepare("UPDATE nobitex_autotrade_positions SET status='pending_close',exit_identifier=:identifier,exit_order_local_id=:local,exit_exchange_order_id=:exchange_id,updated_at=UTC_TIMESTAMP() WHERE id=:id AND status='open'");
        $stmt->execute([':identifier'=>$identifier,':local'=>$created['local_id'] ?? null,':exchange_id'=>$exchangeId !== '' ? $exchangeId : null,':id'=>(int)$victim['id']]);
        if ($stmt->rowCount() !== 1) {
            $this->event($pdo,'error','nobitex.capacity.local_state_conflict',['position_id'=>(int)$victim['id'],'identifier'=>$identifier,'order_local_id'=>$created['local_id'] ?? null]);
        ...
        $context = [
            ...
            'estimated_exit_cost_percent'=>round((float)$victim['estimated_exit_cost_percent'],4),
            'depth_quote'=>(float)$victim['depth_quote'],'volatility_percent'=>round((float)$victim['volatility_percent'],4),
            'dynamic_exit_buffer_percent'=>$buffer,'reference_price'=>$reference,'price_bound'=>$priceBound,'identifier'=>$identifier,
        ];
        $this->event($pdo,'info','nobitex.capacity.sell_submitted',$context);
    ...
    /** Lower score is a better forced-exit candidate. */
    public static function victimScore(array $position): float
    {
        ...
        $vol=max(0.0,(float)($position['volatility_percent']??0.0));
    ...
    public static function dynamicExitBufferPercent(array $market,array $position=[]): float
    {
        ...
        $signal=is_array($market['signal']??null)?$market['signal']:[];$indicators=is_array($signal['indicators']??null)?$signal['indicators']:[];
        ...
        $quote=strtoupper((string)($market['quote_asset']??$position['quote_asset']??'IRT'));
        $shallowThreshold=$quote==='USDT'?25000.0:5000000.0;
        $veryShallow=$quote==='USDT'?5000.0:1000000.0;
        $shallowPenalty=$depth<=0.0?0.35:($depth<$veryShallow?0.20:($depth<$shallowThreshold?0.10:0.0));
        return round(max(0.12,min(1.20,0.08+($spread*1.35)+($vol*0.10)+$shallowPenalty)),4);
    }

    private static function signalEdge(array $signal): float
    {
        foreach(['tradable_net_edge_percent','effective_tradable_net_edge_percent','expected_net_edge_percent'] as $key)if(is_numeric($signal[$key]??null))return(float)$signal[$key];
        return 0.0;
    }
    ...
    private function existingCapacityAttempt(PDO $pdo,int $positionId,int $effective):array
    {
        ...
        $stmt=$pdo->prepare("SELECT identifier,status FROM orders WHERE exchange_name='nobitex' AND side='sell' AND source='autotrade_nobitex_position_limit' AND identifier LIKE :prefix ORDER BY created_at DESC LIMIT 1");
        $stmt->execute([':prefix'=>$prefix.'%']);$row=$stmt->fetch();if(!$row)return['active'=>false];
        $state=strtolower((string)($row['status']??''));return['active'=>in_array($state,['submitting','submitted','pending','filled'],true),'identifier'=>$row['identifier']??null,'status'=>$state];
    }

    private function capacityIdentifier(PDO $pdo,int $positionId,int $effective):string
    {
        $prefix='cap'.$positionId.'m'.$effective.'a';$stmt=$pdo->prepare("SELECT COUNT(*) FROM orders WHERE exchange_name='nobitex' AND source='autotrade_nobitex_position_limit' AND identifier LIKE :prefix");
        $stmt->execute([':prefix'=>$prefix.'%']);return substr($prefix.max(1,(int)$stmt->fetchColumn()+1),0,32);
    }

    private function walletAvailable(array $response,array $assets):float
    {
        $assets=array_map(static fn(string $v):string=>strtoupper($v),$assets);$rows=$response['wallets']??$response['data']??$response;
        if(is_array($rows)&&!array_is_list($rows)&&is_array($rows['wallets']??null))$rows=$rows['wallets'];if(!is_array($rows))return 0.0;
        foreach($rows as $row){if(!is_array($row))continue;$asset=strtoupper((string)($row['currency']??$row['asset']??$row['currencyCode']??''));if(!in_array($asset,$assets,true))continue;foreach(['activeBalance','available','free','balance'] as $key)if(array_key_exists($key,$row)&&is_numeric($row[$key]))return max(0.0,(float)$row[$key]);}
        return 0.0;
    }

    private function assetAliases(string $asset):array{$asset=strtoupper(trim($asset));return match($asset){'TON','GRAM','TONCOIN'=>['TON','GRAM','TONCOIN'],default=>[$asset]};}
    private function floorAmount(float $amount,int $precision):float{$precision=max(0,min(18,$precision));$factor=10**$precision;return floor($amount*$factor)/$factor;}
    ...
    private function intSetting(PDO $pdo,string $key,int $default,int $min,int $max):int{$value=$this->setting($pdo,$key);return is_numeric($value)?max($min,min($max,(int)$value)):$default;}
    private function event(PDO $pdo,string $level,string $event,array $context=[]):void{$pdo->prepare('INSERT INTO nobitex_autotrade_events (level,event_name,context_json,created_at) VALUES (:level,:event,:context,UTC_TIMESTAMP())')->execute([':level'=>$level,':event'=>$event,':context'=>$context===[]?null:json_encode($context,JSON_THROW_ON_ERROR|JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES)]);}
    private function result(string $status,string $reason,array $extra=[]):array{return['status'=>$status,'exchange'=>'nobitex','reason'=>$reason,'time_utc'=>gmdate(DATE_ATOM)]+$extra;}
}
