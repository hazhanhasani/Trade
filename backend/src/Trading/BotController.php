...
{
    public function __construct(private readonly NobitexOrderService $nobitex = new NobitexOrderService()) {}
    ...
    public function status(): array
    {
        ...
        $capacity=$this->nobitexCapacity($settings,$positions);
        ...
        if($lastRun&&isset($lastRun['summary_json'])){
            $lastRun['summary']=json_decode((string)$lastRun['summary_json'],true);unset($lastRun['summary_json']);
            $stamp=(string)($lastRun['finished_at']?:$lastRun['started_at']?:'');if($stamp!=='')$lastRun['time_iran']=IranClock::fromUtc($stamp);
        }

        $nobitexStatus=$this->nobitexStatus($pdo,$quote,$positions,$lastRun)+[
            'role'=>'execution_venue','execution_allowed'=>true,
            'sodium_available'=>function_exists('sodium_crypto_sign_detached'),
            'portfolio_capacity'=>$capacity,
        ];

        return[
            'asset'=>'MULTI','capital_asset'=>'IRT/USDT','quote_priority'=>['IRT','USDT'],
            'execution_mode'=>'nobitex_only','execution_exchange'=>'nobitex',
            'bot_enabled'=>$nobitexStatus['bot_enabled'],'live_execution_enabled'=>$nobitexStatus['live_execution_enabled'],
            'kill_switch'=>$kill,'settings'=>$settings,'position'=>$positions[0]??null,
            'last_run'=>$lastRun,'cron_health'=>$this->cronHealth($lastRun),
            'exchanges'=>['nobitex'=>$nobitexStatus],
            'market_data_sources'=>(new MarketDataCredentialStore())->status(),
            'performance'=>$nobitexStatus['performance'],'time_iran'=>IranClock::nowPayload(),
        ];
    }

    public function setEnabled(bool $enabled):void{$this->setExchangeEnabled('nobitex',$enabled);}

    public function setExchangeEnabled(string $exchange,bool $enabled):void
    {
        NobitexSchema::ensure();$this->executionExchange($exchange);$pdo=Database::connection();
        if($enabled&&!$this->credentialExists($pdo,'nobitex'))throw new \RuntimeException('Nobitex credentials are not configured.');
        NobitexSchema::setBotEnabled('nobitex',$enabled);$this->audit('nobitex'.($enabled?'.autotrade.enabled':'.autotrade.disabled'));
    }

    public function setLiveEnabled(bool $enabled):void{$this->setExchangeLive('nobitex',$enabled);}

    public function setExchangeLive(string $exchange,bool $enabled):void
    {
        $this->executionExchange($exchange);$this->nobitex->setLiveEnabled($enabled);$this->audit('nobitex'.($enabled?'.live.enabled':'.live.disabled'));
    }
    ...
    public function updateSettings(array $input):array
    {
        ...
        $stmt=$pdo->prepare('UPDATE nobitex_autotrade_settings SET quote_asset=:quote,risk_profile=:profile,position_percent=:position,max_position_percent=:max_position,stop_loss_percent=:stop,take_profit_percent=:take,daily_loss_limit_percent=:daily,min_signal_score=:score,cooldown_minutes=:cooldown,updated_at=UTC_TIMESTAMP() WHERE id=1');
        ...
        $this->audit('nobitex.autotrade_settings_updated',$n+['quote_asset'=>$quote]);return$this->status()['settings'];
    }

    public function runNow(string $exchange='nobitex'):array
    {
        $this->executionExchange($exchange);if(!NobitexSchema::botEnabled('nobitex'))return['status'=>'disabled','exchange'=>'nobitex','asset'=>'MULTI'];return(new NobitexAutoTraderEngine())->run();
    }
    ...
    public function recentData(int $limit=25):array
    {
        ...
        foreach($nobitex['signals']as&$row){if(str_ends_with(strtoupper((string)($row['symbol']??'')),'IRT'))$row['price']=NobitexDisplayMoney::quoteValue($row['price']??0,'IRT');if(isset($row['created_at']))$row['time_iran']=IranClock::fromUtc((string)$row['created_at']);}unset($row);
        foreach($nobitex['positions']as&$row){$q=strtoupper((string)($row['quote_asset']??''));if($q==='IRT')$row=NobitexDisplayMoney::moneyFields($row,$q,['entry_price','stop_loss','take_profit','exit_price','realized_pnl']);foreach(['opened_at','closed_at','created_at']as$t)if(!empty($row[$t]))$row[$t.'_iran']=IranClock::fromUtc((string)$row[$t]);}unset($row);
        foreach($nobitex['pnl']as&$row){$q=strtoupper((string)($row['quote_asset']??''));if($q==='IRT')$row=NobitexDisplayMoney::moneyFields($row,$q,['pnl','net_pnl','entry_price','exit_price']);if(isset($row['created_at']))$row['time_iran']=IranClock::fromUtc((string)$row['created_at']);}unset($row);
        return['nobitex'=>$nobitex,'time_iran'=>IranClock::nowPayload(),'nobitex_display_units'=>['IRT'=>'TOMAN','USDT'=>'USDT']];
    }

    private function nobitexStatus(PDO $pdo,string $quote,array $positions,?array $lastRun):array
    {
        $public=$positions;foreach($public as&$row){$q=strtoupper((string)($row['quote_asset']??''));if($q==='IRT')$row=NobitexDisplayMoney::moneyFields($row,$q,['entry_price','stop_loss','take_profit']);if(isset($row['opened_at']))$row['opened_at_iran']=IranClock::fromUtc((string)$row['opened_at']);if(isset($row['updated_at']))$row['updated_at_iran']=IranClock::fromUtc((string)$row['updated_at']);}unset($row);
        return['name'=>'Nobitex','credentials_configured'=>$this->credentialExists($pdo,'nobitex'),'bot_enabled'=>NobitexSchema::botEnabled('nobitex'),'live_execution_enabled'=>$this->nobitex->liveEnabled(),'position'=>$public[0]??null,'active_positions'=>$public,'active_position_count'=>count($positions),'latest_signal'=>$this->latestSignal($pdo),'latest_order'=>$this->latestOrder($pdo),'last_decision'=>$this->lastDecision($lastRun),'performance'=>$this->performance($pdo,$quote)];
    }

    private function activePositions(PDO $pdo):array{return$pdo->query("SELECT id,symbol,asset,quote_asset,amount,entry_price,stop_loss,take_profit,status,opened_at,updated_at FROM nobitex_autotrade_positions WHERE status IN ('pending_open','open','pending_close') ORDER BY id DESC LIMIT 20")->fetchAll();}
    ...
    private function latestSignal(PDO $pdo):?array{$row=$pdo->query('SELECT id,symbol,action,score,price,details_json,executed,created_at FROM nobitex_autotrade_signals ORDER BY id DESC LIMIT 1')->fetch()?:null;if(!$row)return null;if(isset($row['details_json'])){$decoded=json_decode((string)$row['details_json'],true);if(is_array($decoded))$row['details']=$decoded;unset($row['details_json']);}if(str_ends_with(strtoupper((string)($row['symbol']??'')),'IRT')){$row['price']=NobitexDisplayMoney::quoteValue($row['price']??0,'IRT');$row['display_unit']='TOMAN';}if(isset($row['created_at']))$row['time_iran']=IranClock::fromUtc((string)$row['created_at']);return$row;}
    private function latestOrder(PDO $pdo):?array{$row=$pdo->query("SELECT local_id,exchange_order_id,identifier,market_code,side,order_mode,amount,price,status,source,error_text,created_at,updated_at FROM orders WHERE exchange_name='nobitex' ORDER BY id DESC LIMIT 1")->fetch()?:null;if($row&&str_ends_with(strtoupper((string)($row['market_code']??'')),'IRT')&&is_numeric($row['price']??null)){$row['price']=NobitexDisplayMoney::quoteValue($row['price'],'IRT');$row['display_unit']='TOMAN';}if($row&&isset($row['created_at']))$row['time_iran']=IranClock::fromUtc((string)$row['created_at']);return$row;}
    private function lastDecision(?array $lastRun):?array{$summary=is_array($lastRun['summary']??null)?$lastRun['summary']:[];$exchanges=is_array($summary['exchanges']??null)?$summary['exchanges']:[];return is_array($exchanges['nobitex']??null)?$exchanges['nobitex']:null;}
    private function cronHealth(?array $lastRun):array{if(!$lastRun)return['status'=>'unknown','healthy'=>false,'age_seconds'=>null,'message'=>'No cron run has been recorded yet.'];$time=(string)($lastRun['finished_at']?:$lastRun['started_at']?:'');$ts=$time!==''?strtotime($time.' UTC'):false;$age=$ts===false?null:max(0,time()-$ts);$healthy=$age!==null&&$age<=180&&(string)$lastRun['status']!=='failed';return['status'=>$healthy?'healthy':(($age!==null&&$age>180)?'stale':(string)$lastRun['status']),'healthy'=>$healthy,'age_seconds'=>$age,'last_status'=>(string)$lastRun['status'],'message'=>$healthy?'Cron is running normally.':(($age!==null&&$age>180)?'Cron has not completed in the expected window.':'The latest cron run needs attention.'),'last_time_iran'=>$time!==''?IranClock::fromUtc($time):null];}

    private function performance(PDO $pdo,string $quote):array
    {
        [$dayStart,$dayEnd]=IranClock::todayUtcRange();$byQuote=[];
        foreach(['IRT','USDT']as$q){$stmt=$pdo->prepare('SELECT COALESCE(SUM(COALESCE(net_pnl,pnl)),0) FROM nobitex_autotrade_pnl WHERE quote_asset=:q AND created_at>=:start AND created_at<:end');$stmt->execute([':q'=>$q,':start'=>$dayStart,':end'=>$dayEnd]);$today=(float)$stmt->fetchColumn();$stmt=$pdo->prepare('SELECT COALESCE(SUM(COALESCE(net_pnl,pnl)),0) FROM nobitex_autotrade_pnl WHERE quote_asset=:q');$stmt->execute([':q'=>$q]);$total=(float)$stmt->fetchColumn();$stmt=$pdo->prepare("SELECT COUNT(*) FROM nobitex_autotrade_positions WHERE status='closed' AND quote_asset=:q");$stmt->execute([':q'=>$q]);$closed=(int)$stmt->fetchColumn();$stmt=$pdo->prepare("SELECT COUNT(*) FROM nobitex_autotrade_positions WHERE status='closed' AND quote_asset=:q AND realized_pnl>0");$stmt->execute([':q'=>$q]);$wins=(int)$stmt->fetchColumn();if($q==='IRT'){$today=NobitexDisplayMoney::quoteValue($today,$q);$total=NobitexDisplayMoney::quoteValue($total,$q);}$byQuote[$q]=['today_realized_pnl'=>$today,'total_realized_pnl'=>$total,'closed_positions'=>$closed,'winning_positions'=>$wins,'win_rate_percent'=>$closed>0?round(($wins/$closed)*100,2):0.0,'display_unit'=>NobitexDisplayMoney::quoteUnit($q)];}
    ...
    private function executionExchange(string $exchange):string{if(strtolower(trim($exchange))!=='nobitex')throw new \RuntimeException('Nobitex is the only execution exchange.');return'nobitex';}
